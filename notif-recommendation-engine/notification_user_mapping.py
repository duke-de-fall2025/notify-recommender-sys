#this code was directly executed in lambda console
#this code is part of a Lambda function that ranks campaigns based on user embeddings
import boto3
import json
...
def dot_product(vec1, vec2):
    ...
    try:
        ...
        raise Exception(f"Error in dot_product: {str(e)}, vec1={vec1[:5]}, vec2={vec2[:5]}")
...
def get_user_embedding(user_id):
    ...
    embedding = item.get("user_embedding") or item.get("u_embeddings") or item.get("embedding")
...
def handler(event, context):
    try:
        ...
        for campaign in campaigns:
            ...
            campaign_embedding = campaign.get("campaign_embedding") or campaign.get("c_embeddings") or campaign.get("embedding")
